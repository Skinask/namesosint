...
class OSINTTargetChecker:
    ...
    def fetch_static_html(self) -> str:
    ...
    def fetch_dynamic_html(self) -> str:
        """Renders JavaScript using native Termux Chromium/Chromedriver."""
        if not SELENIUM_AVAILABLE:
            print("[!] Selenium is not installed. Falling back to static request.")
        ...
        # Paths commonly used by Termux for chromedriver
        termux_chromedriver_path = "/data/data/com.termux/files/usr/bin/chromedriver"

        try:
            if os.path.exists(termux_chromedriver_path):
                service = Service(termux_chromedriver_path)
                driver = webdriver.Chrome(service=service, options=chrome_options)
            else:
                # Standard system PATH fallback
    ...
    def analyze(self):
        ...
        # 2. Check interactive attributes (alt text, aria-labels for reactions)
